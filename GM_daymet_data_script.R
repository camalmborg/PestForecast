...
spongy_met<-function(dmvar,var){
  ...
  for (i in nsites){
    ...
    for(j in 1:nrow(dm[[i]]$data)){
      dmvar[[i]][as.numeric(as.factor(metyr))[j], dm[[i]]$data$yday[j]]=dm[[i]]$data$var[j]
    }
  }
  return(dmvar)
  save(dmvar,file=paste0(deparse(substitute(dmvar)),".RData"))
}

spongy_met(maxtemp96,var)
